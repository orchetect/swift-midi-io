...
extension MIDIHelper {
    func addNotificationSubscriber(id: String, _ handler: @escaping MIDIManager.NotificationHandler) {
        Task { @MainActor in
            notificationSubscribers[id] = handler
        }
    }

    func removeNotificationSubscriber(id: String) {
        Task { @MainActor in
...
extension MIDIHelper {
    nonisolated func createConnections() {
        createInputConnection()
        createOutputConnection()
    }

    nonisolated func removeConnections() {
...
extension MIDIHelper {
    nonisolated var inputConnection: MIDIInputConnection? {
        midiManager.managedInputConnections[ConnectionTags.midiIn]
    }

    nonisolated func createInputConnection() {
    ...
    nonisolated func updateInputConnection(from prefs: AppPrefs) {
...
extension MIDIHelper {
    nonisolated var outputConnection: MIDIOutputConnection? {
        midiManager.managedOutputConnections[ConnectionTags.midiOut]
    }

    nonisolated func createOutputConnection() {
        // don't re-create the connection if it already exists
    ...
    nonisolated func updateOutputConnection(from prefs: AppPrefs) {
...
extension MIDIHelper {
    ...
    func restorePersistentState(from prefs: AppPrefs) {
