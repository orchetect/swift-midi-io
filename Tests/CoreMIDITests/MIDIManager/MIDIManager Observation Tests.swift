...
struct MIDIManager_Observation_Tests {
    ...
    func devicesStream_Lifecycle() async throws {
        actor TestState {
            var resultCount = 0
            func incResultCount() { resultCount += 1 }
            
            var isFinished = false
            func setIsFinished(_ value: Bool) { isFinished = value }
    ...
    func devicesStream_Data() async throws {
        actor TestState {
            ...
            func updateDevices(_ devices: MIDIDevices) { self.devices = devices }
    ...
    func endpointsStream_Data() async throws {
        actor TestState {
            ...
            func updateEndpoints(_ endpoints: MIDIEndpoints) { self.endpoints = endpoints }
