...
struct EndpointsUpdating_Tests {
    ...
    fileprivate let man1inputTag1 = "SwiftMIDI Endpoints Updating In 1"
    fileprivate let man1inputTag2 = "SwiftMIDI Endpoints Updating In 2"
    fileprivate let man2inputTag1 = "SwiftMIDI Endpoints Updating In 3"
    fileprivate let man1outputTag1 = "SwiftMIDI Endpoints Updating Out 1"
    fileprivate let man2outputTag1 = "SwiftMIDI Endpoints Updating Out 2"
...
struct EndpointsUpdating_Threading_Tests {
    ...
    func endpointsUpdating_threading(arg: TestArgument) async throws {
        ...
        _ = queue1.sync { mw.manager.endpoints.inputs }
        _ = queue1.sync { mw.manager.endpoints.outputs }
        _ = queue1.sync { mw.manager.devices.devices }

        // read endpoints & devices on queue 2
        _ = queue2.sync { mw.manager.endpoints.inputs }
        _ = queue2.sync { mw.manager.endpoints.outputs }
        _ = queue2.sync { mw.manager.devices.devices }
