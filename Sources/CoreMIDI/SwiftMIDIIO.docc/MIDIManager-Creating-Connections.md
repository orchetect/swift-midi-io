...
> In SwiftMIDI, a connection added to the ``MIDIManager`` instance need only be added once in order to form and maintain a unidirectional connection to one or more MIDI ports in the system.
>
...
>
> You may supply the target(s) at the time of creation, or create the connection without any target port(s) and add/remove them later by calling ``MIDIInputConnection/add(outputs:)-([MIDIEndpointIdentity])``, ``MIDIOutputConnection/add(inputs:)-([MIDIEndpointIdentity])``, ``MIDIInputConnection/remove(outputs:)-([MIDIEndpointIdentity])``, or ``MIDIOutputConnection/remove(inputs:)-([MIDIEndpointIdentity])``.
...
As a convenience, SwiftMIDI offers a way to automatically subscribe to receiving events from all MIDI outputs in the system by setting the ``MIDIInputConnectionMode/allOutputs`` mode and filtering out ``MIDIEndpointFilter/owned()`` endpoints owned by the ``MIDIManager``.
...
For simple and powerful event filtering API, see the Event Filters topic in SwiftMIDICore's docs.
