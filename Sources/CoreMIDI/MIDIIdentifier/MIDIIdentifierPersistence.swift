...
public enum MIDIIdentifierPersistence {
    ...
    /// The unique ID will be randomly generated every time the endpoint is created in the system --
    /// with no persistent storage provided.
    ...
    ///
    /// Note that this identifier may change in the event of a collision where another endpoint
    /// already has the identifier. A new random ID will be generated by Core MIDI and .
    ///
    ...
    ///
    /// In the event a collision with an existing MIDI endpoint unique ID in the system, a new
    /// random ID will be generated until there are no collisions.
    /// The ID will then be cached in `UserDefaults` using the key string provided - if the key
